...
def compute_graph_distance(graph1, graph2):
    ...
    for node in source_nodes:
        prob += lpSum(
            [_redirect_flows(j, node) * f_vars[(i, j)] for (i, j) in Fs if node in (i, j)]) == \
...
def _sum_of_weights_not_equal_to_1(graph):
    ...
    """
    sum_of_weights = round(sum([graph.nodes[node]["weight"] for node in graph.nodes]),5)
...
def _equal_amount_of_nodes(graph1,graph2):
    ...
    """
    return len(set(graph1.nodes).intersection(set(graph2.nodes))) != len(set(graph1.nodes))